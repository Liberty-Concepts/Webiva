  ...
  def parse_feature(feature,context) #:nodoc:
      ...
      begin
        ...
        if RAILS_ENV!='production' || myself.editor?
          "<div class='error feature'><b>#{'Feature Definition Contains an Error'.t}</b><br/>#{err.to_s.t}</div>"
        else
          ""
        end
      rescue Radius::UndefinedTagError => err
        if  RAILS_ENV!='production' || myself.editor?
          "<div class='error feature'><b>#{'Feature Definition Contains an Undefined tag:'.t}</b>#{err.to_s.t}</div>"
        else
          ""
        end
      end
